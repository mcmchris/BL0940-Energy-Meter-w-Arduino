...
void setup() {
  ...
  // Configure Serial1 on pins TX=GP0 and RX=GP1
  Serial1.begin(4800);
...
bool _writeRegister(uint8_t address, uint32_t data) {
  ...
  Serial1.write(unlockTxData, sizeof(unlockTxData));
...
bool _readRegister(uint8_t address, uint32_t *data) {
  ...
  while (Serial1.available() != sizeof(rxData)) {
    delay(10);
    if ((millis() - startTime) > timeout)
      break;
  }
  int rxDataLength = Serial1.readBytes(rxData, sizeof(rxData));
